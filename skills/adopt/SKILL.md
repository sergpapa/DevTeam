...
On a non-trivial codebase (~50+ source files), use Graphify. If `graphify --version` fails, install it first — announce you're doing so: `pipx install graphifyy && graphify install` (fall back to `python -m pip install --user graphifyy` if pipx is missing). Then build the knowledge graph: `graphify .` — code extraction is local tree-sitter, zero tokens. Then survey from `GRAPH_REPORT.md` and `graphify query`/`explain` instead of walking files by hand, opening only what the graph flags; the graph also supplies structure evidence for Stage 3's retroactive ADRs. Without Graphify (or on a small repo), read selectively — manifests, README, entry points, config, CI, directory shape; grep instead of full-file reads. Establish:
